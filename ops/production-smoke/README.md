...
### При выпуске, который меняет проверяемый интерфейс

Выкладка (`deploy.yml`) эту копию не трогает: образ собран из того коммита, на
котором стоит каталог проверки. Если выпуск меняет тексты или разметку, на
которые опирается `production-smoke.spec.ts`, старая проверка начнёт падать
каждые десять минут и слать тревоги. Порядок:

1. До выкладки переключите копию на выпускаемый коммит и пересоберите образ
   (`git fetch`, `git checkout --detach <коммит>`, затем `docker compose build
   --pull production-smoke`).
2. Прогоните `bash run-smoke.sh` на текущем продукте. Новая проверка должна
   проходить и на старом интерфейсе; если не проходит — сначала сделайте её
   совместимой с обоими, иначе окно между выкладкой и обновлением даст ложную
   тревогу.
3. После выкладки прогоните `bash run-smoke.sh` ещё раз.

Ручной прогон скрипта тревогу не шлёт: её вызывает только упавшая служба
(`OnFailure`). Упавший прогон оставляет каталог в `failures/`.

Базовый образ Playwright в `frontend-new/Dockerfile.production-smoke` обязан
совпадать по версии с `@playwright/test` из `frontend-new/package-lock.json`,
иначе контейнер не найдёт браузер. Совпадение сверяет юнит-тест
`frontend-new/src/__tests__/production-smoke-image.test.ts`.
