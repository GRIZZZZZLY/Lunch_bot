...
**Новые компоненты (2025-01-08):**

#### `ActivePollActions.tsx`
Главный компонент с действиями для активного голосования.
...
**Возможности:**
- Адаптивная логика для 3 сценариев:
  - Новый пользователь (нет истории голосов)
  - Returning user (есть lastVote, не голосовал в текущем poll)
  - Уже проголосовал в текущем poll
...
#### `LastVoteFeedback.tsx`
Блок оценки прошлого голоса.
...
**Возможности:**
- Показывает последний выбор пользователя
- Кнопки 👍/👎 для оценки
- Optimistic UI (мгновенная реакция)
- Disabled state после оценки
- Haptic feedback

#### `TopDishRecommendation.tsx`
Рекомендация топового блюда для новых пользователей.
...
**Возможности:**
- Показывает топ блюдо с рейтингом
- Процент популярности
- Quick vote одним кликом
- Стилизация под бренд

#### `InviteButton.tsx`
Кнопка приглашения друга через Telegram.
...
**Возможности:**
- Telegram share API
- Deep link с pollId или общая ссылка
- Адаптивный текст (к голосованию / в бот)
- Haptic feedback
