...
- `frontend-new/` — основной выпускаемый интерфейс React/Vite.
- `frontend/` — предыдущий интерфейс, сохранённый как временный резерв для
  сравнения и отката; новые изменения в него не вносятся.
...
| Бюджет `/budget` | `GET /debts`, `/credits`, `/stats`, `/poll-totals/:pollId`, `/order-costs/:pollId`, `/poll-breakdown/:pollId`; `POST /mark-paid`, `/confirm-payment`, `/cancel-mark`, `/mark-all-paid`, `/send-reminder`, `/send-reminders-all`, `/order-costs/:pollId` |
...
- Старый `/api/stats` не дублирует рабочую аналитику: он явно отвечает
  `NOT_IMPLEMENTED`.
- сервер по умолчанию раздаёт `frontend-new`; для временного отката на старый
  интерфейс требуется явное `FRONTEND_DIR=frontend`.
