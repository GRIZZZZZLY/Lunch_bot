...
**Расчет рейтинга:**
- Основан на оценках like/dislike последних 30 дней
- Формула: `(likeCount / totalRatings) * 5`
- Дефолт 4.5 если нет оценок
...
#### `GET /polls/my-last-vote`
Получить последний голос текущего пользователя с рейтингом.
...
- `200` - успешно
- `401` - не авторизован
- `404` - голосов не найдено (data: null)

#### `POST /polls/:pollId/rate`
Оценить свой прошлый голос.

**Parameters:**
- `pollId` (integer) - ID голосования
...
**Коды ответов:**
- `200` - успешно
- `400` - невалидный rating или уже оценен
- `401` - не авторизован
- `404` - голос не найден

#### `GET /polls/:pollId/my-vote-status`
Проверить статус голоса текущего пользователя в голосовании.

**Parameters:**
- `pollId` (integer) - ID голосования

**Response (проголосовал):**
```json
{
  "success": true,
  "data": {
...
**Коды ответов:**
- `200` - успешно
- `400` - уже проголосовал или невалидный menuItemId
- `401` - не авторизован

#### `POST /polls/:pollId/random-vote`
Случайный выбор блюда (рулетка).
...
**Коды ответов:**
- `200` - успешно
- `400` - уже проголосовал
- `401` - не авторизован
- `404` - нет доступных блюд
