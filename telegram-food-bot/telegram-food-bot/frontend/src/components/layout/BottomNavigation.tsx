...
export const BottomNavigation: React.FC = () => {
  ...
  const { user } = useAuth();
  const { activeCount: activePollsCount, activePollId } = useActivePollsCount();
  ...
  const handleNavigation = (path: string) => {
    // Умная навигация для голосования
    if (path === '/vote') {
      if (activePollId) {
        console.log('🔘 [BottomNav] Direct navigation to poll:', activePollId);
        navigate(`/vote/${activePollId}`);
      } else {
        console.log('🔘 [BottomNav] Navigation via VoteRouter (no poll ID)');
        navigate('/vote');
      }
    } else {
      navigate(path);
