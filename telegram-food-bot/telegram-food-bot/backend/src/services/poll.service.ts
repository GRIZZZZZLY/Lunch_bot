    ...
    try {
      ...
      const poll = await prisma.poll.create({
        data: {
          ...
          startedAt: now, // Явно устанавливаем startedAt для синхронизации
        },
      });
      ...
      const expectedEndTime = new Date(now.getTime() + poll.duration * 60 * 1000);
  ...
  static async getActivePolls(): Promise<any[]> {
    try {
      ...
      const activePolls = polls.filter((poll) => {
        ...
        logger.info(`📋 Poll ${poll.id} check:`, {
          status: poll.status,
          startedAt: poll.startedAt.toISOString(),
      ...
      logger.info(`✅ Returning ${activePolls.length} active polls out of ${polls.length} total`);
      
      // Convert ALL BigInt fields to string for JSON serialization
