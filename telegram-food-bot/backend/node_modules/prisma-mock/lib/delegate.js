...
indexes) => {
    ...
    const getFieldRelationshipWhere = (0, fieldHelpers_1.createGetFieldRelationshipWhere)(datamodel, manyToManyData);
    /**
    ...
    const Delegate = (prop, model) => {
        ...
        const nestedUpdate = (args, isCreating, item) => {
            ...
            model.fields.forEach((field) => {
                if (inputData[field.name]) {
                    ...
                    if (field.kind === "object") {
                        ...
                        if (inputFieldData.set) {
                            const { [field.name]: { set }, ...rest } = inputData;
                            const otherModel = datamodel.models.find((model) => {
                                return model.name === field.type;
                            });
                        ...
                        if (inputFieldData.connect) {
                            const { [field.name]: { connect }, ...rest } = inputData;
                ...
                if ((isCreating || inputData[field.name] === null) &&
                    (inputData[field.name] === null || inputData[field.name] === undefined)) {
                    if (field.hasDefaultValue) {
                        if ((0, fieldHelpers_1.isFieldDefault)(field.default)) {
        ...
        const deleteMany = (args) => {
            ...
            deleted.forEach((item) => {
                model.fields.forEach((field) => {
                    ...
                    if (!joinfield)
                        return;
                    const delegate = Delegate((0, fieldHelpers_1.getCamelCase)(field.type), model);
        ...
        const groupBy = (args) => {
            ...
            const havingFields = having ? Object.keys(having).reduce((curr, field) => {
                ...
                const filteredResult = result.filter(group => {
                    for (const [aggregation, conditions] of Object.entries(having)) {
                        for (const [fieldName, operatorValue] of Object.entries(conditions)) {
                            ...
                            if (operator === 'gt' && groupValue <= value)
                                return false;
                            if (operator === 'gte' && groupValue < value)
                                return false;
                            if (operator === 'lt' && groupValue >= value)
                                return false;
                            if (operator === 'lte' && groupValue > value)
                                return false;
                            if (operator === 'equals' && groupValue !== value)
                                return false;
                        }
                    }
                    return true;
