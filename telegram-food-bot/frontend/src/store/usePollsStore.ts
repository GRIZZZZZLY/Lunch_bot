  ...
  updateActiveCount: async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const pollId = response.data[0]?.id || null;
        console.log('[PollsStore] Active polls count:', count, 'Poll ID:', pollId);
...
/**
 * Хук для удобного доступа к active count и poll ID
 */
export const useActivePollsCount = () => {
  const activeCount = usePollsStore((state) => state.activeCount);
  const activePollId = usePollsStore((state) => state.activePollId);
