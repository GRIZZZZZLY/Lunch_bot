...
export const VotingPage: React.FC = () => {
  ...
  console.log('[VotingPage] Poll ID from params:', pollId);
  ...
  // Загрузка данных
  useEffect(() => {
    console.log('[VotingPage] useEffect - loadPollData triggered for pollId:', pollId);
    if (pollId) {
      loadPollData(false); // Первая загрузка - с loading
    } else {
      console.error('[VotingPage] ❌ No pollId in useEffect!');
    }
  }, [pollId]);
  ...
  const loadPollData = async (silent: boolean = false) => {
    console.log('[VotingPage] loadPollData called', { pollId, silent });
    try {
      // Только при первой загрузке показываем loader
      if (!silent) {
        setLoading(true);
        console.log('[VotingPage] Loading started...');
      }

      if (!pollId) {
        console.error('[VotingPage] ❌ Poll ID is missing!');
        throw new Error('Poll ID is missing');
      }

      console.log('[VotingPage] Fetching poll data for ID:', pollId);
      // Загружаем данные голосования
      const pollResponse = await pollsService.getPollById(parseInt(pollId));
      console.log('[VotingPage] Poll response:', {
      ...
      console.error('[VotingPage] ❌ Error loading poll data:', error);
      console.error('[VotingPage] Error details:', {
      ...
      if (!silent) {
        ...
        console.log('[VotingPage] Redirecting to home due to error');
        navigate('/');
      }
    } finally {
      if (!silent) {
        console.log('[VotingPage] Loading finished, setLoading(false)');
        setLoading(false);
      }
    }
  };
  ...
  console.log('[VotingPage] Render state:', { loading, hasPoll: !!poll, pollId });

  if (loading) {
    console.log('[VotingPage] Rendering loading state...');
    return (
      <>
  ...
  if (!poll) {
    console.log('[VotingPage] Rendering "Poll not found" state');
    return (
      <>
        <motion.div
