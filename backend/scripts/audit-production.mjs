 ...
 *    «через» неё же.
 *
 * 2. Два advisory в `mysql2` — драйвере, который нужен Prisma CLI для части
 *    его функциональности, но в этом проекте не используется: проект работает
 *    только с PostgreSQL через `@prisma/adapter-pg`, соединение с MySQL нигде
 *    не открывается. (Какая именно часть CLI тянет mysql2 — по исходникам или
 *    документации Prisma не проверялось; важен факт, что соединение с MySQL
 *    не открывается, а не механизм, которым mysql2 попал в дерево.)
