 ...
 *
 * Блокирует создание только ЖИВОЕ голосование: истёкшее по таймеру
 * `getActivePollInGroup` не возвращает — это мусор от пропущенного тика
 * планировщика, которого на экране уже нет. Саму строку закрывает транзакция
 * `PollService.createPoll`: там стоит частичный уникальный индекс, который
 * иначе не пустит вставку.
 */
