 ...
 *
 * Здесь перечислено ровно то, что система СЕЙЧАС ставит в очередь: все
 * переходы долга — по опросу и по магазинному забегу — и итог «Все оплатили».
 * Мимо очереди по-прежнему уходят сообщения о СОЗДАНИИ долга при расчёте
 * заказа и при закрытии забега: в них реквизиты
 * получателя, которые в БД хранятся зашифрованными, и класть их открытым
 * текстом в очередь нельзя. Тип пополняется вместе с переводом пути, а не
...
export type OutboxMessageType =
  ...
  | 'DEBT_MARK_CANCELLED'
  | 'DEBT_CONFIRMED'
  | 'DEBT_CONFIRMATION_UNDONE'
  | 'DEBTS_ALL_CONFIRMED'
  | 'STORE_RUN_MARKED_PAID';
