...
Последнее обновление: 2026-09-24. Источник функций — фактические маршруты `src/App.tsx`, интерактивные элементы компонентов, документация `frontend-new/docs/` и истории `docs/user-stories/feature-status.csv`. Каждый тест формулируется как: роль → состояние → действие → запрос/Telegram-вызов → интерфейс.
...
| `/` | Правка и удаление расписания | ADMIN | выключенное расписание есть | `home-polls.spec.ts` | подстановка настроек, `PATCH /recurring/:id` с включением, подпись «Автозапуск…»; удаление только после подтверждения («Оставить» ничего не трогает), `DELETE`, форма снова предлагает создать | покрыто |
...
| ссылки из чата | «Заказать», «Открыть меню группы», «Добавить блюдо» | MEMBER, ADMIN | закупка, несколько групп | `team-context.spec.ts` | `?storeRunId=` → `/store-run/:id`; `?groupId=` → меню этой группы; `action=add` → форма блюда | покрыто |
...
| `/budget` | Долг двух людей: отметка и подтверждение | MEMBER + CREATOR | seeded PostgreSQL | `integration.spec.ts` | два браузерных контекста, «Долг закрыт» у должника, «Подтверждено сегодня» у сборщика после перечитывания | интеграция |
| `/` | Опрос двух людей до итога | CREATOR + MEMBER | seeded активный опрос | `integration.spec.ts` | голоса двоих, автозакрытие по последнему голосу, победитель без перезагрузки | интеграция |
| `/store-run/:id` | Закупка двух людей до расчёта | MEMBER + CREATOR | seeded открытый сбор | `integration.spec.ts` | позиция участника, закрытие сбора, цена, расчёт, долг ровно на цену | интеграция |
...
| `/profile` | Отзыв / поддержка | MEMBER | заполнено | `profile-stats-admin.spec.ts` | rating+body, выбранная оценка `aria-pressed`; перехваченный `window.open`, без платежа | покрыто |
