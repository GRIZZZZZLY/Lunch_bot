...
export function DataCleanupCard() {
  ...
  return (
    <div className="card" style={{ padding: 16 }}>
      <div className="font-head" style={{ fontWeight: 700, fontSize: 'var(--text-16)', marginBottom: 12 }}>
        Очистка данных
      </div>
      ...
      {msg && (
        <div style={{ marginTop: 10, padding: '10px 12px', borderRadius: 'var(--radius-block)', background: 'var(--accent-tint)', color: 'var(--accent)', fontSize: 'var(--text-13)' }}>
          {msg}
        </div>
      )}
...
function Block({
  ...
  return (
    <div style={{ background: 'var(--canvas)', borderRadius: 'var(--radius-block)', padding: 12 }}>
      <div style={{ fontWeight: 600, fontSize: 'var(--text-15)', marginBottom: 6 }}>{title}</div>
      <div style={{ display: 'flex', gap: 12, fontSize: 'var(--text-11)', color: 'var(--text-tertiary)', marginBottom: 10 }} className="tnum">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ fontSize: 'var(--text-13)', color: 'var(--text-secondary)' }}>дней</span>
