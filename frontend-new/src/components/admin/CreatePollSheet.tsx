    ...
    >
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 'var(--text-13)', color: 'var(--text-tertiary)' }}>
            {/* Подпись описывает то, что есть сейчас. Раньше при включённом
                переключателе она говорила «выключено», а «каждую рабочую
                неделю» не менялось, когда дни в форме выбирали другие. */}
            {schedule
              ? schedule.isEnabled
                ? `запуск в ${schedule.time}`
                : state.recurring
                  ? 'на паузе — сохраните, чтобы включить'
                  : `на паузе · запуск в ${schedule.time}`
              : state.recurring
                ? 'выберите дни и время ниже'
