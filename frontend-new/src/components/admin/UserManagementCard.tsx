...
export function UserManagementCard() {
  ...
  return (
    <div className="card" style={{ padding: 16 }}>
      ...
      {!isLoading && filtered.length === 0 && <div style={{ color: 'var(--text-tertiary)', fontSize: 'var(--text-13)' }}>Нет пользователей</div>}
...
function UserRow({
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 'var(--radius-block)', background: 'var(--canvas)' }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 'var(--text-15)', fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {name}
          {user.username && <span style={{ color: 'var(--text-tertiary)', fontWeight: 400 }}> @{user.username}</span>}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 3, fontSize: 'var(--text-11)', color: 'var(--text-tertiary)' }}>
