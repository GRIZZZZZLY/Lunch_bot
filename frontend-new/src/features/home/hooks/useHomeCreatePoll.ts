...
export function useHomeCreatePoll() {
  ...
  /* Группа передаётся ЯВНО. Без неё запрос шёл под общим ключом `['menu']`
     со сроком свежести 30 секунд: после переключения команды активный опрос
     подгружался новый, а меню оставалось прежней команды. Последствие видно
     на экране — у нового опроса пропадали варианты ответа, потому что их
     menuItemId не находились в чужом меню. */
  const menuQuery = useMenuItems({ groupId: currentGroupId });
