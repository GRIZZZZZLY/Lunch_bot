...
describe('buildVM — места в рейтинге', () => {
  /* Регрессия. Номер был просто порядковым индексом: при четырёх участниках
     с одним обедом экран показывал 1, 2, 3, 4 — рейтинг там, где все равны,
     и «первое место» доставалось тому, кто раньше попал в Map. */
  it('одинаковое число обедов даёт одно место на всех', () => {
    const vm = buildVM([poll(1, '2026-07-10T10:00:00', [1, 2, 3, 4])], 1, NOW);

    expect(vm.leaders.map((l) => l.rank)).toEqual([1, 1, 1, 1]);
    expect(vm.allTied).toBe(true);
  });

  it('при разных значениях места идут по порядку, а равные делят одно', () => {
    const vm = buildVM(
      [
        poll(1, '2026-07-10T10:00:00', [1, 2, 3]),
        poll(2, '2026-07-11T10:00:00', [1, 2]),
        poll(3, '2026-07-12T10:00:00', [1]),
      ],
      1,
      NOW,
    );

    // 1 → 3 обеда, 2 → 2 обеда, 3 → 1 обед
    expect(vm.leaders.map((l) => [l.lunches, l.rank])).toEqual([
      [3, 1],
      [2, 2],
      [1, 3],
    ]);
    expect(vm.allTied).toBe(false);
  });

  it('одинокий участник не считается «все поровну»', () => {
    const vm = buildVM([poll(1, '2026-07-10T10:00:00', [1])], 1, NOW);
    expect(vm.allTied).toBe(false);
    expect(vm.leaders[0].rank).toBe(1);
  });
});
