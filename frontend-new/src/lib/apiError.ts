...
const BY_CODE: Record<string, string> = {
  ...
  STORE_EXISTS: 'Магазин с таким названием уже есть в этой группе.',
  ...
  /* Раньше на эти две ситуации приходил FINALIZATION_ERROR («проверьте суммы»),
     хотя проверять надо было не суммы: расчёт либо уже закрыт, либо ещё не у
     всех заполнены позиции. Тексты появились вместе с кодами — см.
     backend/src/services/category-order.errors.ts. */
  CALCULATION_COMPLETED: 'Расчёт уже закрыт — суммы изменить нельзя.',
  CALCULATION_NOT_READY:
    'Расчёт закрыть нельзя: не у всех участников заполнены позиции.',
