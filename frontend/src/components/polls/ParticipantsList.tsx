...
export const ParticipantsList: React.FC<ParticipantsListProps> = ({
  ...
  const dishesSummary = participants.reduce((acc, p) => {
    const key = p.dishName;
    if (!acc[key]) {
      acc[key] = {
        name: p.dishName,
        emoji: p.dishEmoji,
    ...
    acc[key].count++;
    acc[key].participants.push(`${p.firstName} ${p.lastName || ''}`.trim());
    return acc;
  }, {} as Record<string, { name: string; emoji?: string; price: number; count: number; participants: string[] }>);
