...
const useCompletedPollWidgetController = ({
  ...
  const pollResult =
    poll.results?.[0] ||
    (poll as PollWithDetails & { result?: PollWithDetails["results"][number] })
      .result;
  ...
  const dishVoteCounts = useMemo(
    () =>
      votes.reduce(
        ...
        {} as Record<
          number,
          { dish: Vote["menuItem"]; count: number; voters: Vote["user"][] }
        >,
      ),
    [votes],
  );
  ...
  const winnerVoteCount = winnerDish
    ? dishVoteCounts[winnerDish.id]?.count || 0
    : 0;
  const winnerPercentage =
    voteCount > 0 ? Math.round((winnerVoteCount / voteCount) * 100) : 0;
...
type CompletedPollController = ReturnType<
  typeof useCompletedPollWidgetController
>;
        ...
        <div className="pt-3 border-t border-gray-200 dark:border-gray-700 space-y-2">
          {dishes.map((item, index) => {
            ...
            return (
              ...
              >
                <div
                  className={`${ICON_SIZES.xl} rounded-lg bg-muted flex flex-shrink-0 items-center justify-center`}
                >
                  <Utensils
                    className={`${ICON_SIZES.md} text-gray-600 dark:text-gray-300`}
                  />
                </div>
                <div className="flex-1 min-w-0">
...
export const CompletedPollWidget = ({
  ...
  const {
    ...
    participants,
    isActuallyCollapsed,
  } = useCompletedPollWidgetController({
    poll,
    showCelebration,
    onCelebrationEnd,
    className,
                ...
                <div className="flex items-center gap-2 mb-3">
                  <div
                    className={cn(
                      ...
                      isDark ? "bg-lavender-500/10" : "bg-primary/10",
                    )}
                  >
                    <span
                      className={cn(
                        "text-xs font-medium flex items-center gap-1",
                        isDark ? "text-lavender-400" : "text-primary",
                      )}
                    >
                ...
                <div className="space-y-2 mb-3">
                            ...
                            {displayVoters.map((voter) => (
                              ...
                              >
                                ...
                                <div
                                  className={cn(
                                    `${ICON_SIZES.md} rounded-full flex items-center justify-center text-[10px] text-white font-semibold flex-shrink-0`,
                                    isDark ? "bg-lavender-500" : "bg-primary",
                                  )}
                                >
                                  {voter.firstName?.[0]?.toUpperCase() || "?"}
                                </div>
                  ...
                  {remainingDishes.length > 0 && (
                    ...
                    >
                      ...
                      >
                        <span>
                          +{remainingDishes.length}{" "}
                          {remainingDishes.length === 1
                            ? "блюдо"
                            : remainingDishes.length < 5
                              ? "блюда"
                              : "блюд"}
                        </span>
                        <m.div
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="truncate text-sm font-semibold text-foreground">
                          ...
                          {pollResult.responsible.lastName &&
                            ` ${pollResult.responsible.lastName}`}
                        </div>
