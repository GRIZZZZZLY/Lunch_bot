...
- зависимости, а также бюджеты Lighthouse и лимиты размера бандла для
  `frontend-new` (прежде эти проверки стояли только на удалённом `frontend`).
...
окружения и SSH-секретов. До обращения к серверу проверяет, что CI для
выпускаемого SHA завершился успешно (`scripts/require-green-ci.sh`).
При неудаче пытается вернуть предыдущий релиз.
