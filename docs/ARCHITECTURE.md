...
`frontend-new` — единственный интерфейс. Прежний каталог `frontend` удалён
2026-08-03: он не участвовал в продакшене (`FRONTEND_DIR=frontend-new`), но
занимал отдельное задание в CI и требовал поддержки при каждой правке общих
токенов и зависимостей. Откат к нему возможен только через историю git.
