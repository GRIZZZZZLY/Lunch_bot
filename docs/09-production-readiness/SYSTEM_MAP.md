...
6. Обратный прокси завершает TLS. Приложение доверяет ровно числу прокси,
   заданному `TRUST_PROXY`. Перед Nginx стоит Cloudflare; настоящий адрес
   клиента Nginx берёт из `CF-Connecting-IP` только для сетей Cloudflare
   (`ops/nginx/cloudflare-realip.conf`).
