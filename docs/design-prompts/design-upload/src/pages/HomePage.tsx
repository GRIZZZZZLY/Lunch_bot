            ...
            >
              <InlineVotingCard
                ...
                onVoteSuccess={() => {
                  ...
                  if (user?.id) {
                    const previousStreak = getUserStreak(user.id);
                    const { streak, milestoneAchieved, newMilestone } = updateStreakAfterVote(user.id);
                    setUserStreak(streak);

                    const today = new Date().toISOString().split('T')[0];
                    const isNewDailyVote = previousStreak.lastVoteDate !== today;

                    if (isNewDailyVote) {
                      setAchievedMilestone(
                        milestoneAchieved && newMilestone
                          ? newMilestone
                          : {
                              emoji: '🔥',
                              title: 'Серия продолжается',
                              message: `${streak.currentStreak} ${streak.currentStreak === 1 ? 'день' : 'дней'} подряд!`,
                            }
                      );
      ...
      <AnimatePresence>
        {isCreatingPoll && (
          <>
            ...
            >
              <GlassCard intensity="ultra" className="overflow-hidden">
